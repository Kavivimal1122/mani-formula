...
# --- UI DISPLAY ---
st.title("🕹️ 91 Game: Pro Predictor with Stick Logic")
...
with col_l:
    st.subheader("Input Result (0-9)")
    grid = [st.columns(5), st.columns(5)]
    for i in range(10):
        r, c = (0, i) if i < 5 else (1, i-5)
        if grid[r][c].button(str(i), key=f"g_{i}"):
            handle_click(i)
            st.rerun()
...
with col_r:
    ...
    st.info(f"Current Pattern Chain: {st.session_state.pattern_chain[-10:] if st.session_state.pattern_chain else 'None'}")
...
if st.session_state.history_data:
    df = pd.DataFrame(st.session_state.history_data)
    # Applying table style (Latest at the top)
    st.table(df.iloc[::-1])
    
    csv = df.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Download History CSV", data=csv, file_name="91_game_history.csv")
else:
    st.info("Click a number in the grid to start recording history.")
